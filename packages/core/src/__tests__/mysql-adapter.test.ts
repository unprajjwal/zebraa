...
describe('MySQLAdapter Integration Tests', () => {
  ...
  beforeAll(() => {
    adapter = new MySQLAdapter(baseConfig);
  });
  ...
  it('should connect to dummy_ecommerce database and fetch schema', async () => {
    const ecomAdapter = new MySQLAdapter({
      ...baseConfig,
  ...
  it('should connect to dummy_analytics database and fetch schema', async () => {
    const analyticsAdapter = new MySQLAdapter({
      ...baseConfig,
  ...
  it('should execute SELECT query with parameters', async () => {
    const res = await adapter.executeQuery('SELECT id, title FROM posts WHERE view_count >= ?', {
      params: [50],
      rowLimit: 100,
    });
  ...
  it('should execute DML write query (INSERT / DELETE) safely', async () => {
    const insertRes = await adapter.executeQuery(
      "INSERT IGNORE INTO tags (name, slug) VALUES ('TestTagMySQL', 'test-tag-mysql')"
    );
    expect(insertRes.columns).toEqual([]);
    expect(insertRes.rows).toEqual([]);

    const deleteRes = await adapter.executeQuery("DELETE FROM tags WHERE slug = 'test-tag-mysql'");
