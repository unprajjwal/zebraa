...
describe('MSSQLAdapter Tests', () => {
  let adapter: MSSQLAdapter;
  let isLiveServerAvailable = false;

  beforeAll(async () => {
    adapter = new MSSQLAdapter(baseConfig);
  ...
  it('should instantiate MSSQLAdapter via registry createAdapter', () => {
    const regAdapter = createAdapter('mssql', baseConfig);
    expect(regAdapter).toBeInstanceOf(MSSQLAdapter);
  });

  it('should return error when testing connection with invalid password', async () => {
    const invalidAdapter = new MSSQLAdapter({
      ...baseConfig,
  ...
  it('should test connection successfully if server is live or report connection failure', async () => {
  ...
  it('should fetch schema info cleanly querying INFORMATION_SCHEMA and sys tables', async () => {
    if (!isLiveServerAvailable) {
  ...
  it('should execute SELECT query with parameters', async () => {
    ...
    expect(res.rows[0]).toEqual([1, 'hello_mssql']);
  });
  ...
  it('should execute SELECT query with Postgres-style $1 parameters', async () => {
    ...
    expect(res.rowCount).toBe(1);
    expect(res.rows[0]).toEqual(['mssql_test']);
  });

  it('should execute explainQuery using SHOWPLAN', async () => {
    if (!isLiveServerAvailable) {
  ...
  it('should fetch table stats querying sys.dm_db_partition_stats', async () => {
    if (!isLiveServerAvailable) {
