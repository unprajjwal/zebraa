...
describe('Desktop Main IPC Postgres Handlers Integration', () => {
  beforeAll(() => {
    setupIpcHandlers();
  });

  it('should handle connections:test IPC', async () => {
  ...
  it('should handle connections:create, connections:list, schema:get, query:execute, and table:stats IPC', async () => {
    const invoke = (ipcMain as any)._invoke;

    // 1. Create connection
    const conn = await invoke('connections:create', {
      name: 'Test Postgres IPC',
      type: 'postgres',
      host: 'localhost',
      port: 5432,
      database: 'zebraa',
    ...
    expect(conn.name).toBe('Test Postgres IPC');
